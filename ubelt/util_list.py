...
class chunks(Generic[VT]):
    ...
    def __init__(self,
        ...
        if total is None:
            try:
                total = len(cast(collections_abc.Sized, items))
            except TypeError:
    ...
    def noborder(items: collections_abc.Iterable[VT],
        ...
        for chunk in chunks_with_sentinals:
            yield [cast(VT, item) for item in chunk if item is not sentinel]
    ...
    def cycle(items: collections_abc.Iterable[VT],
        ...
        for chunk in chunks_with_sentinals:
            yield [cast(VT, item) if item is not sentinel else cast(VT, next(bordervalues))
                   for item in chunk]
    ...
    def replicate(items: collections_abc.Iterable[VT],
        ...
        chunks_with_sentinals = zip_longest(*copied_iters, fillvalue=sentinel)
        for chunk in chunks_with_sentinals:
            filt_chunk = [cast(VT, item) for item in chunk if item is not sentinel]
...
@overload
def take(items: collections_abc.Sequence[VT],
         indices: collections_abc.Iterable[int],
         default: object = util_const.NoParam) -> collections_abc.Generator[VT, None, None]:
    ...


@overload
def take(items: collections_abc.Mapping[KT, VT],
         indices: collections_abc.Iterable[KT],
         default: object = util_const.NoParam) -> collections_abc.Generator[VT, None, None]:
    ...


def take(items: collections_abc.Sequence[VT] | collections_abc.Mapping[KT, VT],
...
class IterableMixin(Generic[VT]):
    """
...
class OrderedIterableMixin(IterableMixin[VT]):
    compress = compress
    argunique = argunique
    window = iter_window
