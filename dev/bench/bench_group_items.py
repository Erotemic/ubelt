...
def benchmark_template():
    ...
    @register_method
    def groupitems_simple_append(items):
    ...
    @register_method
    def groupitems_groupby_and_extend_v1(items):
        pair_list = ((keyfunc(item), item) for item in items)
        id_to_items = defaultdict(list)
        # Is this more efficient? Seems like the answer is no.
        for key, grouper in groupby(pair_list, key=get_0):
            id_to_items[key].extend(grouper)

    @register_method
    def groupitems_groupby_and_extend_v2(items):
        id_to_items = defaultdict(list)
        # Is this more efficient? Seems like the answer is no.
        # even when we optimize for the case of key being a callable
        # EXCEPT in the case where there are large numbers of consecutive items.
        for key, grouper in groupby(items, key=keyfunc):
            id_to_items[key].extend(grouper)

    def generate_input(input_size, num_unique_items):
        """
        input_size, num_unique_items = 10, 10
        """
        import random
        rng = random.Random()
        pool = range(num_unique_items)
        group_ids = rng.choices(pool, k=input_size)
        items = []
